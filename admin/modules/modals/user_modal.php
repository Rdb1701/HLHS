...
$sql = "SELECT user_type_id, tname FROM user_type";
$result = mysqli_query($db, $sql) or die ("Bad SQL: $sql");

$opt1 = "<select class='form-control' name='type' id = 'add_type' required>";
$opt1 .= "<option value='' selected hidden>Select User Type</option>";
  while ($row = mysqli_fetch_assoc($result)) {
    $opt1 .= "<option value='".$row['user_type_id']."'>".$row['tname']."</option>";
  }
...
$opt2 = "<select class='form-control' name='type' id = 'edit_type' required>";
$opt2 .= "<option value='' selected hidden>Select User Type</option>";
  while ($row = mysqli_fetch_assoc($result)) {
    $opt2 .= "<option value='".$row['user_type_id']."'>".$row['tname']."</option>";
  }
