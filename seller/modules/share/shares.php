...
WHERE pro.user_id = '".$_SESSION['seller']['owner_id']."'
AND changedBy = '".$_SESSION['seller']['seller_id']."'
AND property_status = 0
";
...
foreach($property as $key => $value){
    ...
    $total_share = $value['price'] * ($_SESSION['seller']['commission'] / 100);

    $total_shares = '<span class="text-white bg-success" style="padding: 3px 8px; border-radius: 5px;">'.'P '.number_format($total_share).'</span>';

   $data['data'][] = array($value['type_name'], $value['location'], $value['date_inserted'],'P '.number_format($value['price']),$_SESSION['seller']['commission'].'%',$total_shares);
}
